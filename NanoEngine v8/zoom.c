...
int main(int argc, char const *argv[]) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (mode3D) {
            ...
            if (x > 50 && y > 34 && x < GetScreenWidth() && y < GetScreenHeight() && !UI_Menu_Open) {
                DrawRectangle(x, 34, 2, GetScreenHeight(), GRAY);
                DrawRectangle(50, y, GetScreenWidth(), 2, GRAY);
            }
            if (!mode3D) {
        ...
        DrawRectangle(0,0,50,GetScreenHeight(), DARKGREEN);
        ...
        DrawRectangle(50, 0, GetScreenWidth(), 34, DARKGREEN);
